    ...
    def __init__(self, build_prop: BuildProp):
        ...
        try:
            self.build_description = self.get_first_prop(
                BUILD_DESCRIPTION,
                default=fingerprint_to_description(self.build_fingerprint) if self.build_fingerprint else None)
        except ValueError as e:
            print(f'[{e}] Cannot Parse {self.build_fingerprint}')
        # Parse arch
        self.arch = None
        ...
        self.device_uses_dynamic_partitions = self.get_first_prop(DEVICE_USES_DYNAMIC_PARTITIONS, data_type=bool_cast,
                                                                  default=False)
        self.device_uses_virtual_ab = self.get_first_prop(DEVICE_USES_VIRTUAL_AB, data_type=bool_cast, default=False)
        self.device_uses_system_as_root = self.get_first_prop(DEVICE_USES_SYSTEM_AS_ROOT, data_type=bool_cast,
                                                              default=False)
        ...
        self.vendor_build_security_patch = self.get_first_prop(BUILD_VENDOR_SECURITY_PATCH,
                                                               default=self.build_security_patch)
